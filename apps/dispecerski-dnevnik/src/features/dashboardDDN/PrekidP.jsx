import { useGetPrekidP } from "./useGetPrekidP";
import Spinner from "@tis/ui/Spinner";
import Empty from "@tis/ui/Empty";

function PrekidP({ dogId }) {

  const { isLoading, prekidp } = useGetPrekidP(dogId);

  console.log("Dohvaćen PrekidP:", prekidp);

  if (isLoading) return <Spinner />;
//   if (error) return <div>Greška: {error.message}</div>;
  if (!prekidp) return <Empty resourceName="догађај" />;
