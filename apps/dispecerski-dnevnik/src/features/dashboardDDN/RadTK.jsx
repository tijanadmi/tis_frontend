import { useGetRadTK } from "./useGetRadTK";
import Spinner from "@tis/ui/Spinner";
import Empty from "@tis/ui/Empty";

function RadTK({ dogId }) {

  const { isLoading, radtk } = useGetRadTK(dogId);

//   console.log("Dohvaćen RadTK:", radtk);

  if (isLoading) return <Spinner />;
//   if (error) return <div>Greška: {error.message}</div>;
  if (!radtk) return <Empty resourceName="догађај" />;
