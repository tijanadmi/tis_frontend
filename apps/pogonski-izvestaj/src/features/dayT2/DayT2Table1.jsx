...
function DayT2Table() {
  const { dogadjaji, isLoading, count } = useListDayT2();
  // const [searchParams] = useSearchParams();

  if (isLoading) return <Spinner />;
  // console.log("Dnevni T1:", dogadjaji);
  // console.log('count:', count);
  if (count === 0) return <Empty resourceName="дневни Т2" />;
  ...
  return (
    <Menus>
      <Table columns="0.3fr 0.3fr  0.5fr 0.5fr 1fr 2.5fr 2.5fr 1fr 1.5fr">
        <Table.Header>
          <StackedH>
            <span>Ст.</span>
          </StackedH>
