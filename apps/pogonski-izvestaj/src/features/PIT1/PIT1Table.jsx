...
function PIT1Table() {
  const { dogadjaji, isLoading, count } = useListPIT1();
  // const [searchParams] = useSearchParams();

  if (isLoading) return <Spinner />;
  // console.log("Dnevni T1:", dogadjaji);
  // console.log('count:', count);
  if (count === 0) return <Empty resourceName="погонски Т1" />;
  ...
  return (
    <Menus>
      <Table columns="0.3fr  0.5fr 0.5fr 1fr 2.5fr 2.5fr 0.6fr 1.2fr 1.2fr">
        <Table.Header>
          <StackedH>
            <span>Бр.</span>
          </StackedH>
